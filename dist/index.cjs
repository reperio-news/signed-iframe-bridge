...
__export(index_exports, {
  ...
  RefreshTimeoutError: () => RefreshTimeoutError,
  SignedIframeBridgeError: () => SignedIframeBridgeError,
  TokenExpiredError: () => TokenExpiredError,
  TokenVerificationError: () => TokenVerificationError,
  createToken: () => createToken,
  decodeToken: () => decodeToken,
  exportPKCS8: () => import_jose3.exportPKCS8,
  exportSPKI: () => import_jose3.exportSPKI,
  generateKeyPair: () => generateKeyPair,
  importPKCS8: () => import_jose3.importPKCS8,
  importSPKI: () => import_jose3.importSPKI,
...
async function createToken(privateKey, algorithm, payload, ttl, options) {
  let builder = new import_jose.SignJWT(payload).setProtectedHeader({ alg: algorithm }).setIssuedAt().setExpirationTime(`${ttl}s`);
...
async function verifyToken(token, publicKey, algorithm, options) {
  ...
  const { payload } = await (0, import_jose.jwtVerify)(token, publicKey, verifyOptions);
  return payload;
}
...
async function generateKeyPair(algorithm = "ES256") {
  const { publicKey, privateKey } = await (0, import_jose2.generateKeyPair)(algorithm);
